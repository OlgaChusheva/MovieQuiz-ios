...
//  MovieQuizTests
//
//  Created by Ольга Чушева on 14.04.2023.
//

import Foundation // фреймворк для тестирования
import XCTest // приложение для тнстирования

@testable import MovieQuiz

class MovieLoaderTests: XCTestCase {
    func testSuccestLoading() throws {
        // Given
        let stubNetworkClient = StubNetworkClient(emulateError: false) // говорим, что не хотим эмулировать ошибку
        let loader = MoviesLoader(networkClient: stubNetworkClient)
        
        //When
        // так функция загрузки фильмов асинхронное нужно ожидание
        ...
        loader.loadMovies { result in
            ...
            case .failure(_):
                ...
                XCTFail("Unexpexted failyre") // функция проваливает тест
            }
        }
  ...
  func testFailureLoading() throws {
    ...
    let expectation = expectation(description: "Loading expectation")
      ...
      loader.loadMovies { result in
          ...
          case .success(_):
              //сравниваем данные с тем, что мы предпологали
              XCTFail("Unexpexted failyre")
          case .failure(let error):
              //мы не ожидаем что пришла ошибка; если она появится надо будет провалить тест
              XCTAssertNotNil(error)
              expectation.fulfill() // функция проваливает тест
          }
      }
      waitForExpectations(timeout: 1)
  }
